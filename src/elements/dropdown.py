...
class Dropdown(toggle.Toggle):
    def __init__(self, button_object: object, option_buttons: list[object], option_names: list[str], font_colour: tuple[int], font_size: int, font_name: str | None = None, initial_option: int = 0, start_active: bool = False, antialias: bool = False) -> None:
...
class RectDropdown(Dropdown):
    def __init__(self, surface: pygame.Surface, option_names: list[str], x: int, y: int, background_colour: tuple[int], width: int, height: int, font_colour: tuple[int], font_size: int, font_name: str | None = None, y_offset: int = 10, initial_option: int = 0, corner_radius: int = -1, start_active: bool = False, antialias: bool = False) -> None:
        button_object = button.RectButton(surface, x, y, background_colour, width, height, corner_radius=corner_radius)
        option_buttons = self.create_buttons(surface, len(option_names), x, y, background_colour, width, height, y_offset, corner_radius)
    ...
    def create_buttons(self, surface: pygame.Surface, num_options: int, x: int, y: int, colour: tuple[int], width: int, height: int, y_offset: int, corner_radius: int) -> list[button.RectButton]:
        buttons = []
        for i in range(1, num_options + 1):
            new_y = y + i * (height + y_offset)

            btn = button.RectButton(surface, x, new_y, colour, width, height, corner_radius=corner_radius)
            buttons.append(btn)
